...
def train(model, n_epochs, pad_idx, optimizer, scheduler, train_loader, val_loader, dataset, wandb_instance, len_epoch, log_freq=10):
    loss_fn = torch.nn.CrossEntropyLoss(ignore_index=pad_idx)
    min_loss = 100

    for epoch in range(n_epochs):
        losses = 0
        for i, (tgt, _) in enumerate(tqdm(inf_loop(train_loader), total=len_epoch)):
            model.train()
            tgt = tgt.to('cuda')
            tgt_input = tgt[:, :-1]
            ...
            if i % (len_epoch // log_freq) == 0 and i > 0:
                wandb_instance.log({
                    'train_loss': losses / i,
            ...
            if i == len_epoch:
                ...
                wandb_instance.log({
                    'train_loss': losses / len_epoch,
                    'val_loss': val_loss,
                    'lr': scheduler.get_last_lr()[0]
                }, step=cur_step)
                print(f'epoch: {epoch}, train_loss: {(losses / i):.3f}, val_loss: {val_loss:.3f}')
                break
